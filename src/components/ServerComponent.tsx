...
function ServerComponent<T,>({ baseUrl, componentProps, children, componentRoute }: { baseUrl: string, componentRoute: string, componentProps?: T, children?: React.ReactNode }) {
  const [cache, setCache] = useState(initialCache);

  const props = flattenObject(componentProps ?? {});

  const searchParams = new URLSearchParams({...props, __component_route: componentRoute}).toString();
